...
export default function DashboardPelajar() {
  ...
  useEffect(() => {
    axios
      ...
      .then((result) => {
        axios
          .get("http://127.0.0.1:8000/api/getKelasByIdUser/" + JSON.parse(Cookies.get("user")).id, {
            headers: {
    ...
    <div className="bg-slate-200">
      ...
      <div className={loading ? "h-screen" : "h-min-screen"}>
        {loading ? (
          ...
          <div className='flex bg-[url("/BackgroundHome.jpg")] bg-no-repeat bg-cover'>
            <Sidebar data={data} active={0} />
            <div className="flex justify-end w-screen py-6 pr-[70px]">
              <p className="py-3 font-bold text-xl text-white">{"Welcome " + JSON.parse(Cookies.get("user")).nama}</p>
            </div>
          </div>
        )}
      </div>
